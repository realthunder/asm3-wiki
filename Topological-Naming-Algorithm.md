...
def makESHAPE(result_shape, mapper, input_shapes, opcode):
    ...
    for index_name,name_map in names.items():
        ...
        postfix = ';:{}{}{};{};:T{}:{}'.format(
                name_type,first_index,postfix,opcode,first_tag,len(first_name))
        ...
        # it will be named as 
        #
        #    Edge2;:G1(2_Edge3:2,3_Edge4:2);XTR;:T1:4
        #
        ...
        #
        # The ending ;:T1 marks the tag of the first source shape, and the
        # following :4 is the length of the first source element name. This
        # ending mark can be used to easily trace back the first source shape
        # element.
    ...
    for upper_type,element_type in (('Face','Edge'),('Edge','Vertex')):
        ...
        for index_name,name_map in names:
            for upper_mapped_name,index in name_map.items():
                ...
                if upper_tag:
                    postfix += ';:T{}:{}'.format(
                            upper_tag,len(upper_mapped_name))
    ...
    for element_type,lower_type in (('Edge','Vertex'),('Face','Edge')):
        for i,sub_shape in enumerate(result_shape.getSubShapes(element_type)):
            ...
            if lower_tag:
                postfix += ';:T{}:{}'.format(lower_tag,len(first_name))
...
The modified top cylinder face is similarly names as `Face2;:M2;FUS;:T2:5`

The middle smaller face is a modification from two input elements, and is named
as `Face6;:M(Face2;:T2:5);FUS;:T1:5`. Ignoring what's inside the bracket, we get
`Face6;:M();FUS;:T1:5`, and this is similarly named as the first modified face
originated from the `Cube`. The `Face2;:T2:5` inside the bracket is the second source
...
```
Face2;:M2;FUS;:T2:5;:M(Face6;:M(Face2;:T2:5);FUS;:T1:5,Face6;:M2;FUS;:T1:5);RFI;:T2:19
```

This name is difficult for human to read, but easy for program to parse. The ending
`;:T2:19` tells the tag and name length of the first source element. With that, we
can easily find out the source element name being the first 19 characters,
`Face2;:M2;FUS;:T2:5`. Now, strip out the first source name and the trailing
tag and length information, we have

```
;:M(Face6;:M(Face2;:T2:5);FUS;:T1:5,Face6;:M2;FUS;:T1:5);RFI
 |  |                             | |                 |   |
 |  |                             | |                 |   -- op code, indicates
 |  |                             | -------------------      refine operation
 |  |                             |           |
...
`Edge5;:T1:5.Edge5`. The trailing `.Edge5` indicates the old style indexed
...
`Edge5;:T1:5`, with `1` being the Cube's object ID, i.e. `Edge5` from Cube 1.
...
`Edge5;:T1:5` into `Edge5;:M;FUS;:T1:5`. `TopoShape` offers function called
...
def getRelatedElements(shape,name):
    ...
    tag,length = getTagAndLengthFromName(name)
    if not tag:
        return
    ...
    if not tag2:
        ...
        # algorithm
        dehashed += ';:T' + str(tag) + ':' + len(dehashed)
...
naming, reflecting how names are constructed. For example, the refined face
...
#a8;:M#a7;RFI;:T2:3
 |     |  |    | |
 |     |  |    | -- The length is now changed to be length of the hashed name, 
 |     |  |    |    i.e. '#a8'
 |     |  |    |
...
```
`tuple(sourceObjName, sourceElementName, list[intermediateSteps...])`
```
...
naming algorithm, is general enough that they work for `Connect` without
